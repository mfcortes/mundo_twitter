...
sub open_db()
{
	...
	$archivo_DB="Base_Twits_".$tipo_consulta."_".$numeroproceso.".db";
...
sub registraDB()
{
    ...
    if (!exists($memo{$llave}))
    {
		...
		{
			$query = "insert into twits_hashtag 	(idtwits,usuarios_nickName,estado,descTwits,fechatwits,esretwits,retwitscount,favoritecount,respuestatwitsoriginal,id_respuestatwitsoriginal,lat_loc,long_loc,type_loc,url_imagen,follower_count,fechainsert,hashtag_desc_hashtag) values (\"$key\",\"$criterio\",\"ing\",\"$mensaje\",STR_TO_DATE(\"$fecha_at\",\"%a %b %d %H:%i:%s %Y\"),$es_retwits,$q_retwits,$favorite_count,\"$nick_twitsoriginal\",\"$id_respuestatwitsoriginal\",\"$latitud_location\",\"$longitud_location\",\"$tipo_location\",\"$url_imagen\",$follower_count,now(),\"$criterio\")";
...
my $t = Net::Twitter->new(
	...
	while (my $param_consulta = $lista_consultar->fetchrow_array()) 
	{
		...
		foreach my $status (@{$result->{'statuses'}}) 
		{
    		...
    		#&registraDB($status->{id},$param_consulta,$s,$dbh,$fecha_fta,$status->{retweet_count},$rt,$status->{favorite_count},$status->{in_reply_to_screen_name},$status->{in_reply_to_status_id},$status->{coordinates}->{coordinates}[0],$status->{coordinates}->{coordinates}[1],$status->{coordinates}->{type},$status->{user}->{profile_image_url_https},$status->{user}->{followers_count},$tipo_proceso);
